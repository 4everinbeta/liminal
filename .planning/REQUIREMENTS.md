...
- [x] **POLISH-02**: User can see CapacitySummary (hours remaining, task count) while in Focus mode, not just Planning mode
- [ ] **POLISH-03**: User sees meaningful fallback text in the impact pill (e.g., "short task") when a task has no estimated_duration set
- [x] **POLISH-04**: User's restored (soft-deleted) tasks are enqueued in the offline mutation queue when offline and synced on reconnect
