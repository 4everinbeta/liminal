...
must_haves:
  truths:
    ...
    - "No abstract 1-100 numeric inputs visible - replaced with presets or auto-calc"
  artifacts:
...
3. **Form structure - expanded (when showAdvanced=true):**
   ...
   - **CRITICAL: Replace ALL abstract 1-100 numeric inputs with presets (addresses SIMPLIFY-05):**
     - **Priority:** "Today" / "This Week" / "Later" buttons (map to 90/60/30 priority_score)
     - **Value:** "Quick Win" / "Standard" / "Big Project" buttons (map to 90/50/30 value_score)
     - **Effort/Duration:** "Quick (15m)" / "Medium (30m)" / "Long (60m)" / custom minutes input
   - Remove any `<input type="number" min="1" max="100">` for value_score or effort_score
...
5. **Styling:**
   ...
   - Style preset buttons as pill/toggle group (one selected at a time)
...
- "Add details" reveals due_date, duration presets, value presets, description fields
- NO numeric 1-100 input fields for value_score or effort_score visible anywhere
- Creating task with only title succeeds (smart defaults applied)
  </verify>
  <done>
TaskForm requires only title, shows progressive disclosure for optional fields, uses preset buttons instead of abstract 1-100 numeric inputs for value/effort scores, and auto-applies smart defaults on submission.
  </done>
</task>
...
5. Click "Add New Task" again, click "Add details" - should expand to show due date, duration presets, value presets, description
6. Verify NO numeric 1-100 input fields exist for value_score or effort_score
7. Test quick capture in ChatInterface with "test task 15m !high" - should apply hybrid of user input and smart defaults
</verification>
...
- [ ] NO abstract 1-100 numeric inputs for value_score or effort_score (SIMPLIFY-05)
- [ ] Preset buttons replace numeric inputs: Priority (Today/Week/Later), Value (Quick Win/Standard/Big), Duration (Quick/Medium/Long)
