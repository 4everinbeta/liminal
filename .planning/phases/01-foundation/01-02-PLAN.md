...
Output: A board page where any task can be dragged to any column without interruption. Incomplete tasks show a non-blocking visual indicator (icon/badge) instead of modal.
</objective>
...
<task type="auto">
  <name>Task 3: Simplify edit modal to be optional enhancement</name>
  <files>frontend/app/board/page.tsx</files>
  <action>
Modify the edit modal to be a helpful tool rather than a gate:
...
- Drag-drop still works independently of modal
  ...
  <done>
Edit modal is an optional enhancement - all fields optional, helpful messaging, no required validation blocking saves.
  </done>
</task>
...
2. Navigate to /board
3. Create a task with only title (no value/effort) via dashboard
...
8. Save without filling required fields - should succeed
</verification>
