...
Note: FocusToggle.tsx is NOT modified in this plan. The focus/plan toggle is implemented directly in page.tsx header. FocusToggle.tsx remains as-is for potential future use or can be removed in a separate cleanup task.
</objective>
...
2. **Restructure header** to emphasize mode toggle (replaces any use of FocusToggle component):
```typescript
...
3. **Focus Mode view** (when isFocusMode = true):
   ...
   - Remove import/usage of FocusToggle component (toggle is now in header)
...
NOTE: Do NOT import or use FocusToggle.tsx - the toggle is implemented inline in the header. FocusToggle.tsx is left unchanged for now (can be removed in future cleanup).
  </action>
  <verify>
...
- No import of FocusToggle component in page.tsx
  </verify>
  <done>
Dashboard loads focus mode by default, provides clear Focus/Plan toggle in header, consolidates planning view to single column, preserves context on toggle. FocusToggle.tsx is unused but not deleted.
  </done>
</task>
...
- [ ] FocusToggle.tsx is NOT imported/used (toggle is inline in header)
